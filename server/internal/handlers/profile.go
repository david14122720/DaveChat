...
func (h *ProfileHandler) List(c echo.Context) error {
	...
	rows, err := h.DB.Query(
		"SELECT id, username, email, avatar_url, status, last_seen FROM profiles WHERE id != ? ORDER BY username ASC",
		userID,
	)
	if err != nil {
	...
	for rows.Next() {
		...
		if lastSeen.Valid {
			p.LastSeen = lastSeen.Time.Format("2006-01-02T15:04:05Z")
		}
...
func (h *ProfileHandler) Get(c echo.Context) error {
	...
	var lastSeen sql.NullTime
	err := h.DB.QueryRow(
		"SELECT id, username, email, avatar_url, status, last_seen FROM profiles WHERE id = ?", id,
